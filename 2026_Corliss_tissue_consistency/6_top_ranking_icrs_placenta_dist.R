...
xlabel = "Grand Mean Hemi-meth. Beta Dist.    "
...
df_pdist_HUVEC_PLAC = as.data.frame(icr_mean_diff$HUVEC_Placenta)
df_pdist_CD14_PLAC = as.data.frame(icr_mean_diff$CD14_Placenta)

# df_pdist_all <- (df_pdist_HUVEC_PLAC + df_pdist_CD14_PLAC)/2

# Summarize the range of distances across all patients for each ICR 
df_pdist_summary = data.frame( mean = (rowMeans(df_pdist_HUVEC_PLAC) + 
                                       rowMeans(df_pdist_CD14_PLAC))/2)

# Standard deviations combined with sqrt of sum of squares
df_pdist_summary$sd <- sapply(1:nrow(df_pdist_HUVEC_PLAC), FUN = function(x) 
  sqrt( sd(as.matrix(df_pdist_HUVEC_PLAC[x,]))^2 +
        sd(as.matrix(df_pdist_CD14_PLAC [x,]))^2) )
df_pdist_summary$sem <- df_pdist_summary$sd/sqrt(nrow(df_pdist_HUVEC_PLAC))
...
df_pdist_summary$icr_confidence = factor(df_pdist_summary$icr_conf, 
                                         levels = c("1", "2", "3"),
...
g2a
save_plot(paste0(output_dir_path,"/top_icrs_hemi_dist_placenta_1_50.jpg"), g2a,
...
g2b <- ggplot(data = df_pdist_summary[51:100,], aes(y = icr_id, x = abs(mean)) ) + 
  ...
  theme_classic() + theme(legend.position = "none")
g2b
save_plot(paste0(output_dir_path,"/top_icrs_hemi_dist_placenta_51_100.jpg"), g2b,
          base_width = 3, base_height = 7)
...
g2c
save_plot(paste0(output_dir_path,"/top_icrs_hemi_dist_placenta.jpg"), g2c,
